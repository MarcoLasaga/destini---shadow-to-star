...
export const outfitController = {
  ...
  async history(req: AuthRequest, res: Response, next: NextFunction) {
    try { sendSuccess(res, await outfitService.history(req.user!.userId, req.user!.token)) } catch (error) { next(error) }
  },
  async feedback(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const { isSaved, isWorn, rating, note, eventType } = req.body
      ...
      sendSuccess(res, await outfitService.updateFeedback(req.params.id as string, req.user!.userId, { isSaved, isWorn, rating, note, eventType }, req.user!.token))
